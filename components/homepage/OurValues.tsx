    ...
    <div className='w-full my-32 px-6 xl:px-[5.25rem] md:py-52'>
      <div className='max-w-[1440px] flex flex-col items-center justify-center  mx-auto'>
        <h2 style={{  fontFamily: 'Neutro Outline, sans-serif', fontStyle: 'normal'}} 
          className="text-3xl md:text-6xl lg:text-7xl xl:text-8xl 2xl:text-[117.69px] font-bold uppercase text-white text-center">
            Our Values
          ...
          {values.map((value, index) => (
            <div key={index} className='flex flex-col items-center justify-center'>
              ...
              <div className='mt-6 text-white'>
                <p className='text-sm md:text-2xl lg:text-3xl text-center'>{value.title}</p>
                <p className='text-[5.74px] md:text-lg lg:text-[19px] mt-1 md:mt-3 text-center'>{value.description}</p>
              </div>
            </div>
          ))}
